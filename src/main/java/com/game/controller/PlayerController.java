...
public class PlayerController {
    ...
    public ResponseEntity<PlayerInfo> createPlayer(@RequestBody PlayerInfo info) {
        if (info.name == null || info.name.isEmpty() || info.name.length() > 12) return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
        if (info.title == null || info.title.length() > 30) return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
